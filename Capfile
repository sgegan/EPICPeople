...
EOF
  ...
  desc "Check for un-committed remote changes"
  task :remote_changes do
    changed = capture("cd #{current_path} && git status --porcelain")
    if (changed != '' )
      raise Capistrano::Error, "un-committed changes in current deployment, bailing!"
    end
  end
